...
func run() -> void:
	...
	menu.setup(head)
	var entries: Array = []
	for i in 8: entries.append({"id": str(i), "label": str(i)})
	menu.set_page("Test", entries)
	...
	menu.set_page("Short menu", entries.slice(0, 5))
	menu.present({"open": true, "selected_index": 2, "progress": 0.7, "dialangle_degrees": 170.0})
	check(is_equal_approx(menu._canvas.angle, 144.0), "Short-menu pinch indicator matches the five visible sectors")

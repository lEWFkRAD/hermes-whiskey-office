...
Shipped-path restoration: see companion-reproducibility.md for the recorded command,
asset hash, evidence limits, and separately preserved unshipped experiment.
"""
import argparse
import hashlib
import json
...
def replace_head(body, source, target_height=0.325, center_y=0.02):
    ...
    bmesh.ops.bisect_plane(bm, geom=geometry, dist=0.00001, plane_co=(0, 0, 1.422), plane_no=(0, 0, 1), clear_outer=True, clear_inner=False)
    remnants = []
...
def main():
    ...
    head_report = replace_head(mesh, args.head, args.head_height, args.head_center_y) if args.head else None
